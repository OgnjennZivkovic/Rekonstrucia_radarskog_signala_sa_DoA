...
def main():
    ...
    stack = load_stacker(args.ensemble) # ucitava obuceni stack
    cfg = stack.cfg
    if args.data:
        cfg.data_csv = args.data
    out = args.results or cfg.results_dir
    os.makedirs(out, exist_ok=True)
    #Ucitava vrednosti za test
    splits, stats, test_scen = build_arrays(cfg)
    Xte, Yte, Mte = splits["test"]
    print(f"Loaded ensemble {cfg.members}; test pulses: {len(Xte)}")

    member_preds = stack.member_predictions(Xte) # predikcije svih clanova
    ens_pred = stack.ensemble_predict(Xte, member_preds) # kombinovanje svih predikcija
